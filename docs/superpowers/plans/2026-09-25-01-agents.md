...
| `AGENTS.md` | 2 | one sentence: five roles read `github-needs` |
...
- Modify: `AGENTS.md:37-39` (review addition D; this one sentence is the cluster's only change outside `.agents/` and `.claude/agents/`; `CLAUDE.md` is a symlink to it)
...
It writes `version` in each of their `plugins/<name>/plugin.json` files and
each `plugins/<name>/CHANGELOG.md`, and nothing else writes them. Nobody
edits those by hand either, for the same reason.
**The catalogue index carries no version at all**: `.claude-plugin/marketplace.json`
````
...
````text
  ...
  and `CHANGELOG.md`, and no other file.
````
...
````text
  and `conformance.md` says by whom. A wrong sentence in one is a defect of
  the release job, in the repository that runs it — never judged here.
  `version` in the other five `plugins/<name>/plugin.json` files and each
  `plugins/<name>/CHANGELOG.md` are machine-written too, by the release-please
  workflow, and are never judged here either.
````
...
- [ ] **Step 7: `implementer` checks staged `CHANGELOG.md` files**
...
Expected: the first prints `1` for each file. The second prints nothing. The third prints `1`: the howp sentence "Nobody edits any of the three by hand, ever." is still there, unchanged. The fourth prints only `plugins/triz/CHANGELOG.md`: the Implementer's pattern catches a changelog and passes a README. The fifth prints `1` for each file: the howp sentences stay.
...
git add .agents/rules/conformance.md .agents/rules/claims.md .agents/rules/slop.md \
  .agents/skills/slop-police/SKILL.md .agents/skills/pipeline-law/SKILL.md .agents/skills/implementer/SKILL.md
git commit -F - <<'EOF'
chore: treat what release-please writes as release-written

release-please writes version and CHANGELOG.md for five packages, so nobody edits them by hand.
The rules, the pipeline's forbidden paths and the Implementer's staged-path check now say so.
...
## Remaining notes for the coordinator

- `AGENTS.md`, under "What no change does", still says "**Versions**, on the three files a release job in another repository writes". After task 12 the "Versions" section also covers release-please's files in this repository. This plan may change only the `github-needs` sentence of `AGENTS.md`, so that bullet is left for whoever owns it.
- Release-please also rewrites `.release-please-manifest.json`. Addition C named only `version` and `CHANGELOG.md`, so task 12 does not list the manifest file as release-written.
