...
The line is the start of a longer line in each file: `issue-court` continues " You run unattended once a day", `tracker-clerk` continues " You run unattended once a day,", `repo-police` continues " Your job is to keep it looking". Replace only the quoted prefix, in all three files, with:
