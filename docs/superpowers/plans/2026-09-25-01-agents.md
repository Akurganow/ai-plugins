...
- Tasks 8, 9 and 10 name paths that other plans create: `tools/regenerate.sh` (plan 02), `plugins/howp/skills/forecast/` (plan 04), the README's `Install` section (plan 06). All plans land on one branch before merge, so the names are true at merge.
...
| :-- | :-- | :-- |
| `.agents/rules/claims.md` | 1 | diary bullet deleted |
| `.agents/rules/slop.md` | 1, 9, 10 | diary protection gone; the fence and the opening name the regeneration script; howp path |
| `.agents/rules/conformance.md` | 8, 9, 10 | vendor manifest bullet; "Text only"; howp path |
...
| `.agents/skills/slop-police/SKILL.md` | 1, 4, 9 | diary protections; fingerprint line; two programs |
...
| `.agents/skills/pipeline-law/SKILL.md` | 7, 9 | cites the definition; parts; two programs; verification |
...
| `.agents/skills/implementer/SKILL.md` | 1, 9 | diary clause; regeneration in the exit criteria |

Checked and left unchanged: `.agents/prompts/base.md`, `.agents/modes/maintainer.md`, `.agents/policies/repository.yaml`, `.agents/rules/unattended.md`, `.agents/skills/spec-reviewer/SKILL.md`, and the other seven files under `.claude/agents/`. None carries text the spec changes.
...
Expected: the first prints nothing. The second prints one line. The third prints line 89's bullet, unchanged. The fourth prints `1`.
...
Expected: the first prints four lines. The second prints nothing. The third prints one line. The fourth prints `yaml OK`.

- [ ] **Step 8: Run the conformance check**
...
```bash
git add .agents/skills/github-needs/SKILL.md .agents/manifest.yaml
git commit -F - <<'EOF'
...
- **The nine reads**, each as run with what it printed, or as not run with
  why. Never omitted, never summarised as "all clean" without the outputs.
- **The external sources**, each as read, with the file and commit where a
  repository copy was read, or as blocked, with the reply the environment
  gave.
- **One line saying what you could not check**: the callers are not in the
  tree, so nothing here says whether anything fires a role, still
  points at one, or has ever fired.
...
commit you analysed in it. Where any read did not run or any source was
blocked, it reads `Filed nothing. PATROL INCOMPLETE at <sha> — <n> reads or
sources not run.` instead, and never the first: a patrol that could
not run a read audited less than it claims, and
`.agents/rules/unattended.md`'s rule is that a check not run is reported as
not run rather than as passing.
````
...
Expected: 22 lines across the three files.
...
fingerprint and by nothing else. The label is what makes closed issues
findable by listing, which is why it is never removed from an issue it was
applied to.
````
...
fingerprint and by nothing else. The label is what makes closed issues
findable by listing, which is why it is never removed from an issue it was
applied to. Read the auditor's open and closed issues with the same care —
a paragraph it has already filed under its own fingerprint is not yours to
file again under another name. Write the list to `$RUN/do-not-report.md`
````
...
Expected: nine lines, none containing "machine population".
...
of its five kinds, measured the way that kind prescribes, and must not fall
under "What is protected". A case the repository auditor filed is judged by
...
- Every automated finding ends with a fingerprint line naming the role
  that filed it, and the fingerprint is the issue's identity: same problem,
  same file, same fingerprint, across runs. Read the body, never the title
  alone, to know which role filed a case.
- The filing label `police-report` is shared by every filer, so it names
  the population and not the filer. Which issues are a role's own is
  settled by its fingerprint and by nothing else; an `audit:*` label beside
  it says what kind of finding it is, and nothing keys on it.
...
````text
stop. Where they match, take `issue=` only if it reads as a positive integer
and names an issue in the machine population, and try that issue in `$R`.
...
Expected: the first prints nothing. The second prints `6`. The third prints two lines. The fourth prints `front matter OK`.
...
The police roles run weekly and file findings under one protocol, and
three parts of it are load-bearing here. Every automated finding ends
with a fingerprint line naming the role that filed it, and the fingerprint
is the issue's identity: same problem, same file, same fingerprint, across
...
Expected: the first prints nothing. The second prints `3`.
...
`police-report` is the one label that bounds what the pipeline sees. With
...
1. List the open issues in the machine population carrying
   ...
   Clerk, on a finding the court sustained and it re-derived as still live,
   and the owner, on any issue in that population he wants built. **You
   never apply it to anything**, and you are still the only role that
   removes it.
````
...
Expected: the first prints nothing. The second and third print one line each. The fourth prints `3` for `pipeline-law` and `3` for `pipeline-clerk`.
...
**Files:**
- Modify: `.agents/rules/conformance.md:84-94`
...
Expected: the first prints nothing. The second and third print one line each: the new rule, and the Codex sentence kept. The fourth prints `2`.
...
their references, one check, one regeneration script, the workflows — and
...
You are the Slop Police for the repository **Akurganow/ai-plugins** — a
public marketplace of agent plugins: every package under `plugins/` is an
Agent Plugins 1.0.0 package, the repository holds text only, and it runs
two programs: the conformance check and the regeneration entry point. You
run unattended once a week, and you do not change any file.
````
...
Expected: the first prints nothing. The second prints exactly these seven files:
...
and nothing else; `project.md` comes from task 8. The count is six paths; the list above is the whole expected output.
...
````text
4. Get a reader from the plugin to a **sourced** install path for every
   surface the root README's Install section lists — either in the
   plugin README itself or by pointing at the root README's Install
   section. A command stated without a source is the graver finding; a
````
...
### Task 11: Verify the cluster as a whole (spec §10, the `.agents/` line)
...
Expected: the first prints the heading and at least two `external-disagreement` lines. The second prints at least `10`.
...
- [ ] **Step 6: Every front matter and the manifest parse**
...
- [ ] **Step 7: The conformance check and a clean tree**
...
1. **Parts are filed by the Pipeline Clerk.** The spec names three filers. A part without `police-report` would sit outside the population, where no role reaches it. Task 7 makes the Clerk a filer and replaces `pipeline-part:` with a `police-fingerprint: pipeline-clerk …` line. The owner confirms or chooses otherwise.
2. **Closed issues under an old marker.** By §3.3 they leave the population when closed. A finding a person declined under an old marker can then be filed once more under the new line. The plan follows the spec. The alternative is to count old markers in any state for the do-not-report list only.
3. **The self-repair of a missing `police-report` label goes.** A filer finds its own issues only through the label now, so it cannot see one filed without it. Instead a filer that cannot confirm the label files nothing (task 2). An open old-marker issue without the label is outside the population.
4. **The owner's `pipeline/intake`.** He could put it on any issue. After task 7 the Clerk takes it only from issues in the population.
5. **The Agent Police had no fingerprint value.** Its skill named a marker but no form. Task 4 proposes `<path>::<concept>::<pass>`.
6. **`manifest.yaml` has no description of the role.** Spec §3.2 asks for one to be updated. The plan updates the skill's and the binding's descriptions, and the `shared.github-needs` note. `AGENTS.md` says "four analysis roles share `github-needs`"; it belongs to no cluster.
7. **Kept, and possibly stale.** The kept Codex sentence in `conformance.md` says Codex "publishes no plugins documentation to read first". Research 09 read such documentation at `developers.openai.com/plugins/build/plugins.md`. Spec §4.2 keeps the sentence.
8. **Outside the spec, not changed.** `policies/repository.yaml`'s `paths.allow` lacks `CONTRIBUTING.md`, `SUPPORT.md`, `SECURITY.md` and `docs/**`. `conformance.md` "Versions" and `claims.md` name only howp's three release-written files. Release-please (§7.1) will also write five `version` fields and `CHANGELOG.md` files.
9. **The release job's path.** Plan 04's rename moves `references/commands.md`, which a job in how-possible writes. Task 10 follows the rename here; the job is plan 04's and plan 07's concern.
