...
| :-- | :-- | :-- |
| `.agents/rules/claims.md` | 1, 12 | diary bullet deleted; the release workflow's files |
| `.agents/rules/slop.md` | 1, 9, 10, 12 | diary protection gone; the fence and the opening name the regeneration script; howp path; the release workflow's files |
...
The file's shape stays: a `description`, then `paths.allow` as globs, then the comment explaining why `deny` is empty, then `redact` and `confirmations`. Only `allow` grows, and the `description` names what it now covers. The review listed thirteen paths. Existing globs already cover nine of them, so adding them again would be two entries for one path:
...
The four the list adds are `CONTRIBUTING.md`, `SUPPORT.md`, `docs/**` and `cog.toml`, cocogitto's configuration (spec §7.1). Two more files that other plans change are outside every glob, so this task adds them too. `SECURITY.md` is rewritten by plan 06 (spec §6.2). `.gitattributes` is created by plan 02 (conventions, `tools/regenerate.sh`).

- [ ] **Step 1: Run the test and see it fail**

```bash
grep -nE '"(CONTRIBUTING|SUPPORT|SECURITY)\.md"|"docs/\*\*"|"cog\.toml"|"\.gitattributes"' .agents/policies/repository.yaml
```
...
```bash
grep -cE '"(CONTRIBUTING|SUPPORT|SECURITY)\.md"|"docs/\*\*"|"cog\.toml"|"\.gitattributes"' .agents/policies/repository.yaml
$PY -c "
import yaml
d = yaml.safe_load(open('.agents/policies/repository.yaml'))
a = d['paths']['allow']
assert len(a) == len(set(a)) == 13, a
assert d['paths']['deny'] == [], d['paths']['deny']
print('policy OK')"
grep -c 'A deny entry here would make that' .agents/policies/repository.yaml
```

Expected: the first prints `6`. The second prints `policy OK`: thirteen entries, no duplicates, `deny` still empty. The third prints `1`: the comment giving the reason stays.
...
The policy now allows the community files, docs, SECURITY.md, .gitattributes and cog.toml.
...
### Task 12: What the release workflow writes is release-written too (review addition C)
...
**Interfaces:**
- Consumes: spec §7.1. Plan 07 writes `.github/workflows/release.yml`, which runs cocogitto for every package except howp. Its bump hook writes `version` into `plugin.json` and the `.claude-plugin/plugin.json` copy; cocogitto writes each `CHANGELOG.md`.
- Produces: nothing a later task reads.

The howp sentences stay as they are. Each edit adds text after them, and none touches the lines task 10 edits. The pipeline's forbidden-path table and the Implementer's staged-path check follow in the same task. Otherwise `conformance.md` would forbid a hand edit to `CHANGELOG.md` that the pipeline still allows. `AGENTS.md`'s "Versions" bullet follows too.

- [ ] **Step 1: Run the test and see it fail**

```bash
grep -rn 'workflows/release.yml\|CHANGELOG' .agents/rules .agents/skills/pipeline-law/SKILL.md .agents/skills/implementer/SKILL.md
...
```bash
grep -c 'workflows/release.yml' .agents/rules/conformance.md .agents/rules/claims.md .agents/rules/slop.md .agents/skills/pipeline-law/SKILL.md
grep -rn 'release-please' .agents AGENTS.md
grep -n 'cocogitto release workflow\|three files a release job' AGENTS.md
test -L CLAUDE.md && echo symlink-kept
grep -n 'three release-written\|three files the release' .agents/skills/slop-police/SKILL.md
...
Expected: the first prints `1` for each file. The second prints nothing. The third prints one line, the new bullet. The fourth prints `symlink-kept`. The fifth prints nothing. The sixth prints `1`: the howp sentence "Nobody edits any of the three by hand, ever." is still there, unchanged. The seventh prints only `plugins/triz/CHANGELOG.md`: the Implementer's pattern catches a changelog and passes a README. The eighth prints `1` for each file: the howp sentences stay.
...
git commit -F - <<'EOF'
chore: treat what the release workflow writes as release-written

The cocogitto release workflow writes version, its vendor copy and CHANGELOG.md for five packages, so nobody edits them by hand.
...
grep -c 'workflows/release.yml' .agents/rules/conformance.md .agents/rules/claims.md .agents/rules/slop.md
```
...
8. **Added.** The policy allows the new files (task 11, addition B). The release-written files include what the cocogitto release workflow writes (task 12, addition C).
...
- The "Versions" bullet of `AGENTS.md` names what the how-possible release job writes and what the cocogitto release workflow writes (task 12, step 8).
- The owner chose cocogitto over release-please (spec §7.1). Task 12 names `.github/workflows/release.yml`. For the five packages, the release-written files are `version` in `plugin.json`, its `.claude-plugin/plugin.json` copy and `CHANGELOG.md`. Task 11 allows `cog.toml`.
