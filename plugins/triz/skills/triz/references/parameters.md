...
`sources.md` names, and a variant is shown where those transcriptions
differ. The parameters describe physical systems. The software reading
beside each is this skill's own, written for the mapping step, and is not
Altshuller's text. Use it to find the parameter that plays the same role
in the system at hand, not as a definition.
...
| 10 | Force (Intensity) | Cost of one operation: the work of one render, one query, one transformation |
...
| 13 | Stability of the object's composition | Consistency: state in agreement across parts, invariants kept, contracts preserved |
| 14 | Strength | Strictness of guarantees: type safety, validation rigour, access control |
| 15 | Duration of action of moving object | How long a running process stays healthy: a session, a connection, an uptime |
| 16 | Duration of action by stationary object | How long stored things stay valid: cache lifetime, token expiry, artifact reuse |
...
| 37 | Difficulty of detecting and measuring | Difficulty of testing and measuring: hard-to-mock dependencies, unobservable behaviour |
